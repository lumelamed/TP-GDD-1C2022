...
GROUP BY V.CODIGO_VUELTA, S.CIRCUITO_CODIGO, TS.SECTOR_TIPO, E.CODIGO_ESCUDERIA, P.CODIGO_PILOTO,
		COSMICOS.PK_Del_Tiempo(CA.CARRERA_FECHA),
...
ORDER BY MAYOR_CONSUMO_COMUSTIBLE DESC
GO
...
SELECT  E.ESCUDERIA_NOMBRE, DM.CUATRIMESTRE,DM.ANIO , SUM(P.TIEMPO_PARADA)/SUM(CANTIDAD_PARADAS) 'Tiempo Promedio Paradas'
...
WITH CIRCUITOS_PELIGROSOS AS (
    SELECT I.CODIGO_CIRCUITO, C.CIRCUITO_NOMBRE, S.DESCRIPCION, 
	MAX(I.CANTIDAD_INCIDENTES) MAXIMA_CANTIDAD_INCIDENTES, DT.ANIO, ROW_NUMBER() 
									OVER (
										PARTITION BY DT.ANIO
										ORDER BY DT.ANIO
									) AS NROFILA 
	FROM COSMICOS.BI_HECHOS_INCIDENTES I
	JOIN COSMICOS.BI_DIMENSION_TIEMPO DT ON DT.CODIGO_TIEMPO = I.CODIGO_TIEMPO
	JOIN COSMICOS.BI_DIMENSION_CIRCUITO C ON C.CIRCUITO_CODIGO = I.CODIGO_CIRCUITO
	JOIN COSMICOS.BI_DIMENSION_TIPO_SECTOR S ON S.CODIGO_TIPO_SECTOR = I.CODIGO_TIPO_SECTOR
	GROUP BY I.CODIGO_CIRCUITO, C.CIRCUITO_NOMBRE, S.DESCRIPCION, DT.ANIO
)
SELECT CODIGO_CIRCUITO, CIRCUITO_NOMBRE, DESCRIPCION, MAXIMA_CANTIDAD_INCIDENTES, ANIO
FROM CIRCUITOS_PELIGROSOS
WHERE NROFILA <= 3 -- TOP 3
GO
...
SELECT I.CODIGO_ESCUDERIA, E.ESCUDERIA_NOMBRE, S.DESCRIPCION, DT.ANIO,
(SUM(I.CANTIDAD_INCIDENTES)/COUNT(S.CODIGO_TIPO_SECTOR)) PROMEDIO_INCIDENTES
FROM COSMICOS.BI_HECHOS_INCIDENTES I 
JOIN COSMICOS.BI_DIMENSION_TIEMPO DT ON DT.CODIGO_TIEMPO = I.CODIGO_TIEMPO
JOIN COSMICOS.BI_DIMENSION_ESCUDERIA E ON E.CODIGO_ESCUDERIA = I.CODIGO_ESCUDERIA 
JOIN COSMICOS.BI_DIMENSION_TIPO_SECTOR S ON S.CODIGO_TIPO_SECTOR = I.CODIGO_TIPO_SECTOR
GROUP BY I.CODIGO_ESCUDERIA, E.ESCUDERIA_NOMBRE, S.DESCRIPCION, DT.ANIO
GO
