...
CREATE TABLE [COSMICOS].[BI_HECHOS_AUTOS_Y_CARRERAS] (
	...
	CODIGO_ESCUDERIA, CODIGO_COMPONENTE, CODIGO_PILOTO, CODIGO_TIEMPO)
)
GO
...
--			CODIGO_COMPONENTE, CODIGO_PILOTO, CODIGO_TIEMPO, DESGASTE_COMPONENTE, CONSUMO_COMBUSTIBLE, MAXIMA_VELOCIDAD, MEJOR_TIEMPO)
...
SELECT  E.ESCUDERIA_NOMBRE, DM.CUATRIMESTRE,DM.ANIO , SUM(P.TIEMPO_PARADA)/SUM(CANTIDAD_PARADAS) 'Promedio' FROM COSMICOS.BI_HECHOS_PARADAS P
...
SELECT distinct E.ESCUDERIA_NOMBRE,C.CIRCUITO_NOMBRE, DM.ANIO, SUM(P.CANTIDAD_PARADAS) 'Cantidad paradas' FROM COSMICOS.BI_HECHOS_PARADAS P
JOIN COSMICOS.BI_DIMENSION_TIEMPO DM ON DM.CODIGO_TIEMPO = P.CODIGO_TIEMPO
JOIN COSMICOS.BI_DIMENSION_ESCUDERIA E ON E.CODIGO_ESCUDERIA = P.CODIGO_ESCUDERIA
...
go
-- Q7
SELECT TOP 3 CODIGO_CIRCUITO,SUM(TIEMPO_PARADA) TIEMPO_PARADA
FROM COSMICOS.BI_HECHOS_PARADAS P
GROUP BY  CODIGO_CIRCUITO
ORDER BY SUM(TIEMPO_PARADA) DESC

-- Q8
CREATE VIEW COSMICOS.Circuitos_Mas_Peligrosos_Por_Anio AS
SELECT TOP 3 T.ANIO,C.CIRCUITO_NOMBRE FROM COSMICOS.BI_HECHOS_INCIDENTES I
...
GROUP BY  T.ANIO,C.CIRCUITO_NOMBRE
