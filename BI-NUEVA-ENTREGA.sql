...
SELECT A.CODIGO_AUTO,A.AUTO_NUMERO,A.AUTO_MODELO,S.DESCRIPCION,C.CIRCUITO_NOMBRE, MAX(H.MAXIMA_VELOCIDAD) FROM COSMICOS.BI_HECHOS_AUTOS_Y_CARRERAS H
...
SELECT  E.ESCUDERIA_NOMBRE, DM.CUATRIMESTRE,DM.ANIO , SUM(P.TIEMPO_PARADA)/SUM(CANTIDAD_PARADAS) 'Promedio paradas' FROM COSMICOS.BI_HECHOS_PARADAS P
...
SELECT TOP 3 DI.CODIGO_CIRCUITO, DI.CIRCUITO_NOMBRE, SUM(TIEMPO_PARADA) TIEMPO_PARADA
FROM COSMICOS.BI_HECHOS_PARADAS P
JOIN COSMICOS.BI_DIMENSION_CIRCUITO DI ON DI.CIRCUITO_CODIGO = P.CODIGO_CIRCUITO
GROUP BY DI.CODIGO_CIRCUITO, DI.CIRCUITO_NOMBRE
...
SELECT TOP 3 C.CODIGO_CIRCUITO, C.CIRCUITO_NOMBRE, T.ANIO
...
GROUP BY  C.CODIGO_CIRCUITO, C.CIRCUITO_NOMBRE, T.ANIO
