...
CREATE TABLE [COSMICOS].[BI_HECHOS_INCIDENTES] (
	...
	[CANTIDAD_INCIDENTES] int, [CANTIDAD_INCIDENTES_POR_AÑO_EN_CIRCUITO] int,
...
INSERT INTO [COSMICOS].[BI_HECHOS_INCIDENTES] (CODIGO_CIRCUITO, CODIGO_TIPO_SECTOR, CODIGO_ESCUDERIA, 
		CODIGO_TIEMPO, CODIGO_TIPO_INCIDENTE, CANTIDAD_INCIDENTES, CANTIDAD_INCIDENTES_POR_AÑO_EN_CIRCUITO)
SELECT C.CIRCUITO_CODIGO, TS.SECTOR_TIPO, E.CODIGO_ESCUDERIA, 
		...
		(SELECT COUNT(*) FROM COSMICOS.INCIDENTE I 
JOIN COSMICOS.CARRERA CAR ON I.CODIGO_CARRERA = CAR.CODIGO_CARRERA
JOIN COSMICOS.CIRCUITO CIR ON CIR.CIRCUITO_CODIGO = CAR.CODIGO_CARRERA
WHERE CIR.CIRCUITO_CODIGO = C.CIRCUITO_CODIGO AND YEAR(CARRERA_FECHA) = YEAR(CA.CARRERA_FECHA))
...
GROUP BY  C.CIRCUITO_CODIGO, TS.SECTOR_TIPO, E.CODIGO_ESCUDERIA, COSMICOS.PK_Del_Tiempo(CA.CARRERA_FECHA), TI.INCIDENTE_TIPO, CA.CARRERA_FECHA
...
SELECT MAX(MAXIMA_VELOCIDAD) AS VELOCIDAD_MAX, HA.CODIGO_ESCUDERIA, HA.CODIGO_CIRCUITO , DT.ANIO
FROM COSMICOS.BI_HECHOS_AUTOS_Y_CARRERAS HA
JOIN COSMICOS.BI_DIMENSION_ESCUDERIA DE ON DE.CODIGO_ESCUDERIA = HA.CODIGO_ESCUDERIA
JOIN COSMICOS.BI_DIMENSION_CIRCUITO DC ON DC.CIRCUITO_CODIGO = HA.CODIGO_CIRCUITO
JOIN COSMICOS.BI_DIMENSION_TIEMPO DT ON DT.CODIGO_TIEMPO = HA.CODIGO_TIEMPO
GROUP BY HA.CODIGO_ESCUDERIA, HA.CODIGO_CIRCUITO , DT.ANIO
GO
-- Q3
...
create table circuitos_peligrosos(
codigo_circuito int,
nombre_circuito nvarchar(256),
cantidad_incidentes int,
Año int)
GO

create procedure insetar_en_hechos_top3_circuitos_peligrosos_por_anio 
as begin
	declare @anio int
	...
	begin
		insert into circuitos_peligrosos (codigo_circuito, nombre_circuito, cantidad_incidentes, Año)
		select TOP 3  CODIGO_CIRCUITO, CIRCUITO_NOMBRE, CANTIDAD_INCIDENTES_POR_AÑO_EN_CIRCUITO, T.ANIO FROM  COSMICOS.BI_HECHOS_INCIDENTES H 
		JOIN COSMICOS.CIRCUITO C  ON H.CODIGO_CIRCUITO = C.CIRCUITO_CODIGO
		JOIN COSMICOS.BI_DIMENSION_TIEMPO T ON T.CODIGO_TIEMPO = H.CODIGO_TIEMPO
		WHERE T.ANIO = @anio
		GROUP BY  CODIGO_CIRCUITO, CIRCUITO_NOMBRE,CANTIDAD_INCIDENTES_POR_AÑO_EN_CIRCUITO, T.ANIO
		ORDER BY 3 DESC
	fetch next FROM top3_circuitos 
	into @anio
	end
...
exec insetar_en_hechos_top3_circuitos_peligrosos_por_anio 
GO


CREATE VIEW COSMICOS.Circuitos_Mas_Peligrosos_Por_Anio AS
SELECT * from circuitos_peligrosos
GO
...
-- Q9
 ...
 CREATE VIEW COSMICOS.Promedio_incindentes_por_escuderia_por_tipo_de_sector_por_año
 AS
 SELECT DISTINCT H.CODIGO_ESCUDERIA,
 ESCUDERIA_NOMBRE,
 TTS.DESCRIPCION,
 T.ANIO, 
 SUM(CANTIDAD_INCIDENTES) CANTIDAD_INICIDENTES_POR_TIPO_DE_SECTOR,
 ( SUM(CANTIDAD_INCIDENTES)/
 (SELECT COUNT(*) FROM COSMICOS.SECTOR S
 JOIN COSMICOS.TIPO_SECTOR TS ON TS.SECTOR_TIPO = S.SECTOR_TIPO
 JOIN COSMICOS.INCIDENTE I ON I.CODIGO_SECTOR = S.CODIGO_SECTOR
 JOIN COSMICOS.AUTO_POR_INCIDENTE AI ON AI.CODIGO_INCIDENTE = I.CODIGO_INCIDENTE
 JOIN COSMICOS.AUTO_POR_CARRERA AC ON AC.CODIGO_AUTO_POR_CARRERA = AI.CODIGO_AUTO_POR_CARRERA
 JOIN COSMICOS.AUTO A ON A.CODIGO_AUTO = AC.CODIGO_AUTO
 JOIN COSMICOS.ESCUDERIA E ON E.CODIGO_ESCUDERIA =  A.CODIGO_ESCUDERIA
 JOIN COSMICOS.CARRERA C ON C.CODIGO_CARRERA = AC.CODIGO_CARRERA WHERE TS.SECTOR_TIPO = CODIGO_TIPO_SECTOR  AND E.CODIGO_ESCUDERIA = H.CODIGO_ESCUDERIA AND YEAR(CARRERA_FECHA) = T.ANIO ))PROMEDIO_INCIDENTES_POR_SECTOR

 FROM COSMICOS.BI_HECHOS_INCIDENTES  H 
 JOIN COSMICOS.BI_DIMENSION_TIEMPO T ON H.CODIGO_TIEMPO = T.CODIGO_TIEMPO
 JOIN COSMICOS.ESCUDERIA E ON E.CODIGO_ESCUDERIA = H.CODIGO_ESCUDERIA
 JOIN COSMICOS.TIPO_SECTOR TTS ON TTS.SECTOR_TIPO = H.CODIGO_TIPO_SECTOR
 GROUP BY  H.CODIGO_ESCUDERIA,ESCUDERIA_NOMBRE, CODIGO_TIPO_SECTOR,TTS.DESCRIPCION, T.ANIO
