...
DECLARE @codigo_carr int
        DECLARE CURSOR_CARRERAS_N CURSOR FOR 
        SELECT CODIGO_CARRERA FROM COSMICOS.CARRERA
        OPEN CURSOR_CARRERAS_N
        FETCH NEXT FROM CURSOR_CARRERAS_N INTO @codigo_carr
            ...
            BEGIN
                ...
                FETCH NEXT FROM CURSOR_CARRERAS_N INTO @codigo_carr
            END
CLOSE CURSOR_CARRERAS_N
DEALLOCATE CURSOR_CARRERAS_N
...
--FRENO POR AUTO
DECLARE @codigo_carr_f int
        DECLARE CURSOR_CARRERAS_F CURSOR FOR 
        SELECT CODIGO_CARRERA FROM COSMICOS.CARRERA
        OPEN CURSOR_CARRERAS_F
        FETCH NEXT FROM CURSOR_CARRERAS_F INTO @codigo_carr_f
            WHILE @@FETCH_STATUS = 0
            BEGIN
				...
				FROM gd_esquema.Maestra m
				JOIN COSMICOS.AUTO a ON m.AUTO_NUMERO = a.AUTO_NUMERO AND m.AUTO_MODELO = a.AUTO_MODELO
					AND m.CODIGO_CARRERA = @codigo_carr_f
                ...
                FETCH NEXT FROM CURSOR_CARRERAS_F INTO @codigo_carr_f
            END
CLOSE CURSOR_CARRERAS_F
DEALLOCATE CURSOR_CARRERAS_F
